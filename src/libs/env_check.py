...
class EnvCheck:
    ...
    def check_env(self):
        if not os.path.exists(".env"):
            ...
            print(
                ".env file created. Please update the file with your environment variables."
            )
    ...
    def check_answer(self, key: str, optional: bool) -> str:
        ...
        while answer == "":
            if optional:
                answer = input(
                    f"Enter value for {key} - example: {self.example_env[key]}\n(optional, Enter to skip): "
                )
    ...
    def check_done(self, key: str) -> bool:
        ...
        return not (
            key in self.existing_env and self.existing_env[key] == self.example_env[key]
        )
